...
$array = Get-ChildItem ".\" -Filter *.pdf
$cnt = $array.length
$tot_len = 0
$tot_len2 = 0
foreach ($item in $array) {
    ...
    # Redirection to null variable to suppress output
    cpdf.exe -clean -o "$name" "$name" > $null
    cpdf.exe -compress -o "$name" "$name" > $null
    cpdf.exe -squeeze -o "$name" "$name" > $null
    $item2 = Get-Item "$name"
    $len2 = $item2.length
    $tot_len2 += $len2
